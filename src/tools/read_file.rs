    ...
    async fn definition(&self, _prompt: String) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Read the contents of a file from the repository. \
                Returns the full text content. Use this to examine source code, \
...
pub async fn read_file(repo_root: &Path, relative_path: &str) -> Result<String, String> {
...
mod tests {
    ...
    async fn read_existing_file() {
        ...
        let content = read_file(dir.path(), "test.txt").await.unwrap();
    ...
    async fn read_file_too_large() {
        ...
        let result = read_file(dir.path(), "huge.bin").await;
        assert!(result.is_err());
