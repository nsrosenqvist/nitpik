...
- **Critic verification (opt-in)** — `--verify` runs a perspective-diverse critic panel after deduplication: three independent lenses (balanced, soundness, grounding) vote keep/drop on each finding in parallel, dropping a finding only on a majority vote (and a cross-lens-corroborated one only on a unanimous vote). This suppresses probable false positives while voting across diverse lenses guards against any single prompt's blind spots. Add `--show-dropped` to see what the panel discarded and why.
...
- **Quality post-processing** — findings are deduplicated across agents (using shared evidence snippets as one of the merge signals), filtered to only the lines you actually changed, and severity-normalized before they reach you. The result: actionable findings, not LLM noise.
...
> **Performance note:** Compiling the built-in regex rules adds a few seconds of startup time. This cost is paid once per invocation and only when secret scanning is enabled — normal reviews without `--scan-secrets` are unaffected.
