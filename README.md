...
- **Auto profile selection** — `--profile auto` picks reviewers from the diff. Tune the strategy with `--auto-mode heuristic|llm|hybrid` (default `hybrid` uses heuristics first, falls back to a built-in `triage` LLM call only when the heuristics are inconclusive).
