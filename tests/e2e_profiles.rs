...
async fn run_review_collecting_audit(
    ...
    let baseline =
        context::build_baseline_context(repo_path, &diffs, config, false, &[], Vec::new(), None)
            .await;
...
async fn run_review(repo_path: &Path, profile_names: &[&str], config: &Config) -> Vec<Finding> {
    ...
    let agent_defs = if profiles.iter().any(|p| p == "auto") {
        // Always-on lenses plus the heuristic conditional candidates.
        let mut auto_profiles = vec!["security".to_string(), "correctness".to_string()];
        auto_profiles.extend(nitpik::agents::auto::heuristic_lens_candidates(&diffs));
...
async fn e2e_custom_profile() {
    ...
    let baseline =
        context::build_baseline_context(&repo, &diffs, &config, false, &[], Vec::new(), None).await;
    let review_context = ReviewContext {
        diffs,
        baseline,
...
async fn e2e_cache_prior_findings() {
    ...
    let baseline_v1 =
        context::build_baseline_context(&repo, &diffs_v1, &config, false, &[], Vec::new(), None)
            .await;
    ...
    let baseline_v2 =
        context::build_baseline_context(&repo, &diffs_v2, &config, false, &[], Vec::new(), None)
            .await;
